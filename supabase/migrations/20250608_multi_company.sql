...
-- Anon kiosk policies remain store-scoped via existing filters;
-- employees/stores readable for active records only.
...
-- スーパー管理者の例（メールアドレスを書き換えて実行）
-- INSERT INTO company_members (user_id, role)
-- SELECT id, 'super_admin' FROM auth.users WHERE email = 'admin@example.com'
-- ON CONFLICT (user_id) DO UPDATE SET role = 'super_admin', company_id = NULL;
